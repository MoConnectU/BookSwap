...
import { Plus, Check, Bell, LogOut } from 'lucide-react'
...
export default function Profile() {
  ...
  const fetchMyData = async () => {
    ...
    const [{ data: books }, { data: swaps }] = await Promise.all([
      ...
      supabase.from('swap_requests').select('*, books!requested_book_id(title, author), profiles!requester_id(name)').eq('owner_id', user.id).eq('status', 'pending')
    ])
    setMyBooks(books || [])
              ...
              {myBooks.map((b, i) => (
                <Card key={b.id}>
                  ...
                  <div style={{ padding: '0.8rem' }}>
                    <div style={{ fontSize: '0.82rem', fontWeight: 700, color: C.text, marginBottom: 4 }}>{b.title}</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.7rem', color: C.success, fontWeight: 600 }}>
